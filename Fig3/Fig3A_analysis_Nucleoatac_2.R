...
# functions-----
##This function takes peak_bed data.frame that has location of the binding events 
##in the format of chrn, start, end. 
##And another data.frame that has gene names and location of its TSS called TSS_info. 
##It requires to have columns that have name mgi_symbol, TSS.location, and Chromosome_location. 
##Also, it will take how many base pairs away you want to see your binding events. 
##Window will be in the units of kb. And it will return a data.frame that is directly plottable in ggplot2 
##line segment function. 

Colnames <- c("mgi_symbol", "TSS.location", "Chromosome_location")

make_TSS_bindinglocation_table <- function(peak_bed, TSS_info, window){
  ##testing for only one gene for now
  ###Create vectors that will store all the values for the last data frame generation
  gene_name <- vector(mode = "character")
  start_pos <- vector(mode = "numeric")
  end_pos <- vector(mode = "numeric")
  row_pos <- vector(mode = "numeric")
  
  
  for (i in 1:nrow(TSS_info)){
    
    Chromosome <- as.character(TSS_info$Chromosome_location[i]) ##Look on a particular chromosome
    peak_on_the_chr <- peak_bed[peak_bed[, 1] == Chromosome, ]
    
    ##Look for binding events within window
    keep <-  ((TSS_info$TSS.location[i] - window * 1000 < peak_on_the_chr[, 3]) 
              & (peak_on_the_chr[, 3] <= (TSS_info$TSS.location[i] + window * 1000))) |
      ((peak_on_the_chr[, 2] <= TSS_info$TSS.location[i] + window * 1000) 
       & (peak_on_the_chr[, 2] >= TSS_info$TSS.location[i] - window * 1000))
    
    
    gene_name <- append(gene_name, rep(as.character(TSS_info$mgi_symbol[i]), sum(keep)))
    start_pos <- append(start_pos, peak_on_the_chr[keep, 2] - TSS_info$TSS.location[i])
    end_pos <- append(end_pos, peak_on_the_chr[keep, 3] - TSS_info$TSS.location[i]) 
    row_pos <- append(row_pos, rep(i, sum(keep)))
  }
  ###If start is before - window * 1000, then convert it into -window * 10000
  start_pos[start_pos < - window * 1000] <- -window * 1000
  ###If end is after window * 1000, then convert it into window * 10000
  end_pos[end_pos > window * 1000] <- window * 1000
  
  return(data.frame(gene_name, start_pos, end_pos, row_pos))
}
...
#take just the first, or splitting on comma and taking all, did the latter
# hist1.numbers = as.numeric(gsub(",..*","",as.character(hist1$distance2center))); hist1.numbers = na.omit(hist1.numbers)
hist1.numbers = unlist(str_split(hist1$distance2center, ","));hist1.numbers = as.numeric(hist1.numbers[hist1.numbers!= ""])
...
############################################################################
#binding events mapping----

hist = hist1[,c(1:24,25)]
hist = hist1[,c(1:24,26)]
colnames(hist)[ncol(hist)] = "distance2dyad"
peak_bed = hist[which(hist$distance2dyad!=""|hist$distance2dyad!=",,"),-1]
#nucleosome info actually, centered on nucleosomes
TSS_info = data.frame(mgi_symbol = (peak_bed$Gene.Name), TSS.location= peak_bed$Start, Chromosome_location = peak_bed$Chr)

peak_bed$distance2dyad =(gsub("\\s*\\([^\\)]+\\)","",as.character(peak_bed$distance2dyad)))
peak_bed$distance2dyad =as.numeric(gsub(",..*","",as.character(peak_bed$distance2dyad)))
# peak_bed$distance2dyad =as.numeric(gsub(",,..*","",as.character(peak_bed$distance2dyad)))
# remove doubles for now
peak_bed$Start = peak_bed$Start+peak_bed$distance2dyad
peak_bed$End = peak_bed$Start+10
peak_bed = peak_bed[ !is.na(peak_bed$distance2dyad),]

window = .4
output_table = make_TSS_bindinglocation_table(peak_bed, TSS_info, window)

#plotting line in analysis_Nucleoatac_2.R below
